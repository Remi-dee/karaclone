        ...
        <div>
            ...
            <div className='bg-[white] border border-slate-100 mt-5  px-8 rounded-lg py-4 text-[dark] w-full  mx-auto'>
                ...
                <WithdrawalForm amountToRecieveId="amountToWithdraw" headerText="Amount to Withdraw " currency="usd"
                    placeholder="Amount" onAmountChange={handleAmountChange} />
                ...
                {/* submit button */}
                <div className="mt-5">
                    <button className={`${isValid ? 'bg-purple-600 text-white-100' : 'bg-purple-200 text-slate-700' }
                        text-sm w-full py-3 rounded-lg`} disabled={!isValid} onClick={handleWithdrawalSubmit}>
                        {'Continue'}
                    </button>
                </div>
